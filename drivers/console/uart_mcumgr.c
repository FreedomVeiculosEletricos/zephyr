...
#if defined(CONFIG_MCUMGR_TRANSPORT_UART_ASYNC)
static void uart_mcumgr_async(const struct device *const dev,
...
{
#if defined(CONFIG_UART_MCUMGR_RAW_PROTOCOL)
	return uart_mcumgr_send_raw(dev, data, len);
#else
...
static void uart_mcumgr_setup(const struct device *const dev)
{
	...
	uart_rx_enable(dev, async_buffer[0], sizeof(async_buffer[0]),
